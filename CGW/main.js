...
function draw() {
    ...
    gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false, m4.multiply(modelViewProjection,m4.translation(...Object.values(getVertex(spherePosition.u,spherePosition.t)))));
    sphereSurface.Draw();
}
...
window.onkeydown = (e) => {
    ...
    if (e.keyCode == 87) { //w
        spherePosition.u = Math.min(spherePosition.u + 0.1, Math.PI * 2);
    }
    else if (e.keyCode == 65) { //a
        spherePosition.t = Math.max(spherePosition.t - 0.1, -1);
    }
    else if (e.keyCode == 83) { //s
        spherePosition.u = Math.max(spherePosition.u - 0.1, 0);
    }
    else if (e.keyCode == 68) { //d
        spherePosition.t = Math.min(spherePosition.t + 0.1, 1);
    }
}